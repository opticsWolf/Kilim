...
- v0.4.32: menu start dirs survive close — the sidecar rewrite at close dropped shell_cwds (and the git dock keys), so a Start Directory set via the menu died with the session even though it applied immediately. _persist_perspective now re-stashes every sidecar extra, and the configured per-shell dir unconditionally beats the saved live dir on a fresh start.
- v0.4.31: live shell directories end to end — the core now seeds invisible cwd reporting into every spawned shell (cmd via PROMPT, bash via PROMPT_COMMAND with native spelling, PowerShell via a chaining prompt wrapper), so the snapshot cwd tracks cd live instead of freezing at the spawn dir. That one fix heals three symptoms: per-pane live dirs persist at close and restore on launch (sidecar pane_cwds, saved-dir beats shell default beats inherit), the Git dock follows genuine terminal focus changes (initial focus and same-pane moves never re-point), and cwd-relative links resolve live.
- v0.4.30: Git inks pass contrast on all themes — a contrast audit over all 10 themes caught the light status set failing AA on the mid-gray Neutral paper (3.5-4.3, tuned for white only), so the light inks are deepened to pass AA on white and mid-gray alike. A new all-themes test pins it: status letters + body text at WCAG AA 4.5, lanes/pills/dim/link at 3.0, on every theme.
- v0.4.29: Git file list finds renames — the files widget passes -M explicitly, so renames always show as one cyan R row (old arrow new) instead of a red/green delete+add pair whenever user git config disables rename detection; the file diff uses -M too, and renamed files highlight the old side under the old path (it never existed under the new one).
- v0.4.28: Git dock fully theme-aware — every ink the dock paints now follows the code theme: dark/light lane, pill, and file-status sets picked by paper luminance (checked for contrast), message/diff/file text from the theme foreground, dimmed text blended toward the paper, and the sha + selection accents tied to lane zero. All inks resolve synchronously from the theme registry at switch time — widget palettes are never sampled, because Lace re-applies those asynchronously and one-shot paints would otherwise go stale. Fixed along the way: the header repaint after a switch never ran for auto-selected commits (selected index was only tracked on clicks).
- v0.4.27: Git dock follows focus, themes, AA, syntax diffs — the history dock now re-points when focus moves between terminal docks (viewer focus keeps the last terminal's repo); code-theme switches re-paper the graph/lists/message and re-highlight the open diff; the graph paints anti-aliased (curves, dots, pills, text). File diffs are syntect-highlighted through a new highlight_code bridge call (not mordant: that renders Markdown, code was always syntect's job): hunk-tracked spans with green/red washes, first-parent diffs, empty tree at roots, plain line colors past 2000 lines.
- v0.4.26: painted Git graph + file diff inspector — the history list is now a custom-painted graph (own lane routing over the commit DAG: per-lane colors, gradient bezier forks/merges, ref pills, hover row tint + ancestor/descendant rings + identity tooltip, click and Up/Down/Home/End selection) instead of git text output. Below it an inspector: bold message header, status-colored file list (renames show old arrow new), and a colored unified diff preview per file (first-parent diff, empty tree at roots). Also fixed along the way: paint used an instance-level QPalette enum that PySide6 only exposes on the class (AttributeError on first paint).
- v0.4.25: Git History dock — Views > Git History docks a branch-graph commit browser for a local git tree (new git_pane module, git CLI only, no new deps): the graph is git's own --graph output with colored refs plus author/date, click a commit for its full message and file stat, filter by branch (all, current, locals), and a .git watcher refreshes after commits and fetches. Opens on the active terminal's repo (its live dir walked to the top-level), Open picks another tree, and open-state + repo persist in the sidecar so the dock (and its geometry) survives a restart. Qt-only: a Python-side dock with no session pane.
